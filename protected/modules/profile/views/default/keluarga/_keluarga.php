...
$datatipekeluarga = Html::dropDownList('famtype_id', null,ArrayHelper::map($listtype, 'master_id', 'name'), ['class'=>'form-control form-control-primary', 'prompt'=>'CHOOSE '.$model->getAttributeLabel('famtype_id').'']);
$datastatuskeluarga = Html::dropDownList('famstatus_id', null,ArrayHelper::map($liststatus, 'master_id', 'name'), ['class'=>'form-control form-control-primary', 'prompt'=>'CHOOSE '.$model->getAttributeLabel('famstatus_id').'']);
$dataprofesikeluarga = Html::dropDownList('profession_id', null,ArrayHelper::map($listprofession, 'master_id', 'name'), ['class'=>'form-control form-control-primary', 'prompt'=>'CHOOSE '.$model->getAttributeLabel('profession_id').'']);
?>

<div class="card card-absolute">
	<div class="tab-content card-block">
		<div class="card-header bg-primary">
			<h5 class="text-white"><i class="icofont icofont-users"></i> Keluarga</h5>
		</div>
		...
		<div class="row m-b-10">
			<div class="col-md-12 text-right">
				...
				<button onclick='showformkeluarga("", "Tambah")' data-toggle="modal" data-target="#loadmodal" data-keyboard="false" data-backdrop="static" class="btn btn-sm btn-pill btn-primary"><i class="icofont icofont-plus-square"></i> Tambah</button>
...
<script>
    $(document).ready(function() {
		...
		$('#'+table_id+' thead tr:eq(1) th').each( function (i){
			...
			if(nofilter == true){
				...
				if(dpdown == true){
					if(i == 3){
						$(this).html(<?php echo json_encode($datatipekeluarga); ?>);
					}else if(i == 4){
						$(this).html(<?php echo json_encode($datastatuskeluarga); ?>);
					}else if(i == 9){
						$(this).html(<?php echo json_encode($dataprofesikeluarga); ?>);
					}else if(i == 6){
						$(this).html('<select class="form-control form-control-primary" style="padding:.5rem !important;"><option value="">CHOOSE<option value="L">LAKI-LAKI</option><option value="P">PEREMPUAN</option></select>');
					}else{
		...
		$('#'+table_id+'').DataTable({
			...
			"ajax": {
				type: 'POST',
				dataType: "json",
				url: '<?php echo Url::toRoute([Yii::$app->controller->id.'/loaddtkeluarga']); ?>',
    ...
    function showformkeluarga(dataid, dataaction) {
        $('#loadmodal').find('.modal-header .modal-title').html('<i class="icofont icofont-database"></i> Form ' + dataaction + ' Keluarga');

        $.ajax({
            url: '<?php echo Url::toRoute([Yii::$app->controller->id . '/showformkeluarga']); ?>',
            type: 'POST',
            dataType: "html",
            data: {
                dataid: dataid,
