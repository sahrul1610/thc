...
$datajenispelatihan = Html::dropDownList('trg_id', null,ArrayHelper::map($listpelatihan, 'master_id', 'name'), ['class'=>'form-control form-control-primary', 'prompt'=>'CHOOSE '.$model->getAttributeLabel('trg_id').'']);
?>

<div class="card card-absolute">
	<div class="tab-content card-block">
		<div class="card-header bg-primary">
			<h5 class="text-white"><i class="icofont icofont-list"></i> Pelatihan</h5>
		</div>
		...
		<div class="row m-b-10">
			<div class="col-md-12 text-right">
				...
				<button onclick='showformpelatihan("", "Tambah")' data-toggle="modal" data-target="#loadmodal" data-keyboard="false" data-backdrop="static" class="btn btn-sm btn-pill btn-primary"><i class="icofont icofont-plus-square"></i> Tambah</button>
...
<script>
    $(document).ready(function() {
		...
		$('#'+table_id+' thead tr:eq(1) th').each( function (i){
			...
			if(nofilter == true){
				...
				if(dpdown == true){
					if(i == 5){
						$(this).html(<?php echo json_encode($datajenispelatihan); ?>);
					}
		...
		$('#'+table_id+'').DataTable({
			...
			"ajax": {
				type: 'POST',
				dataType: "json",
				url: '<?php echo Url::toRoute([Yii::$app->controller->id.'/loaddtpelatihan']); ?>',
    ...
    function showformpelatihan(dataid, dataaction) {
        $('#loadmodal').find('.modal-header .modal-title').html('<i class="icofont icofont-database"></i> Form ' + dataaction + ' Pelatihan');

        $.ajax({
            url: '<?php echo Url::toRoute([Yii::$app->controller->id . '/showformpelatihan']); ?>',
            type: 'POST',
            dataType: "html",
            data: {
                dataid: dataid,
