...
class IdentityController extends Controller
{
	...
	public function actionLoaddtidentitas() {
        if (Yii::$app->request->isAjax) {
			...
			$datawhere->orderBy(['person_name'=>SORT_ASC, 'identitytype_id'=>SORT_ASC]);
			...
			if($custom_search == 'buttonzip'){
				...
				$filepath = $directory.'/document_identity.zip';
				...
				if(!empty($datazip)){
					foreach($datazip as $ddx=>$drow){
						...
						if(file_exists($file_path) && $drow->url_scan_identity){
							$setslug = $slugify->slugify($drow->person_name.' '.$drow->identitytype_name);
				...
				if($datazipcount > 0){
					...
					$data['url'] = $urilink.'/document_identity.zip?'.time();					
				}else{
					$data['code'] = 200;
					$data['status'] = 'Failed';
