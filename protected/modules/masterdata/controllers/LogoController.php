			...
			if (!empty($dataall)) {
				...
				foreach ($dataall as $index => $value) {
					...
					$data['data'][$index][] = $start + $i;
						...
						$data['data'][$index][] = '<img style="width:150px;" src="'.Logic::getFile($value['name']).'"/>';
		...
		try {
			...
			if (Yii::$app->request->isAjax) {
                ...
                if ($dataaction == 'Hapus') {
					...
					if ($model->upload($old_file, Yii::$app->user->identity->employee->company_name, 'logo')['data'] == true) {
						$model->save();
						$transaction->commit();
