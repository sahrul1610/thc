...
class FaviconController extends Controller
{
	...
	private function _getKey(){
		return $this->id == 'favicon' ? 'favicon' : '';
	}
			...
			if (!empty($dataall)) {
				...
				foreach ($dataall as $index => $value) {
					...
					$data['data'][$index][] = $start + $i;
						...
						$data['data'][$index][] = '<img style="width:50px;" src="'.Logic::getFile($value['name']).'"/>';
		...
		try {
			...
			if (Yii::$app->request->isAjax) {
                ...
                if ($dataaction == 'Hapus') {
					...
					$model->data_file = UploadedFile::getInstance($model, 'name');
					if ($model->upload($old_file, Yii::$app->user->identity->employee->company_name, 'favicon')['data'] == true) {
						$model->save();
						$transaction->commit();
